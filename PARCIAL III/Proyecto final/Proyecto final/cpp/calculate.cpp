	...
	for (auto it = in.cbegin(); it != in.cend();) {
		...
		if (it2 != it) {
			if (lasttok.first == ")" || (opers.find(lasttok.first) == opers.end() && funcs.find(lasttok.first) != funcs.end()) || lasttok.second == -1)
				throw parse_error("Operador faltante", i);
		...
		for (; fit != funcs.end(); ++fit) {
			auto str = in.substr(i, fit->first.length());
			if ((i + fit->first.length()) > in.length()) {
				continue;
			}
			if (opers.find(fit->first) == opers.end() && fit->first == str) {
				break;
			}
		}
		...
		if (*it == ',') {
			if (lasttok.first == "(" || lasttok.first == ",")
				throw parse_error("Argumento faltante", i);
			...
			if (!found)
				throw parse_error("Encontrado ',' no dentro de los argumentos de la funcion", i);
		...
		if (*it == ')') {
			...
			if (!found)
				throw parse_error("Exceso encontrado '('", i);
		...
		throw parse_error("Expresion desconocida encontrada ", i);
	}

	while (!s.empty()) {
		token_t tok = pop(s);
		if (tok.first == "(")
			throw parse_error("Encontrado sin cerrar '('", in.size());
		out.push_back(tok);
	}
...
num_t evalpostfix(postfix_t in) {
	...
	for (token_t& tok : in) {
		if (tok.second == -1) // number
			...
			if (s.size() < tok.second)
				throw runtime_error("No hay suficientes argumentos (tiene " + to_string(s.size()) + ") para la funcion '" + tok.first + "' (se necesita " + to_string(tok.second) + ")");
			else {
				args_t v;
				...
				if (ret.first)
					...
					throw runtime_error("Argumentos inaceptables (" + args.str() + ") para la funcion '" + tok.first + "'");
				}
			}
		}
	}
	...
	if (s.size() == 1)
		...
		throw runtime_error("No se encontro ningun resultado");
}
